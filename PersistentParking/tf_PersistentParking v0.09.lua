...
dataref("tfpp_fltCurrentHeading", "sim/flightmodel/position/psi")
-- dataref("tfpp_R", "sim/flightmodel/position/R",writable)
...
function tfpp_UpdateArray()
	...
	tmpString = tfpp_fltCurrentHeading
	--print(tmpString)
	tfpp_PersistentParkingParams[tmpIndex] = tmpString
	--print(tfpp_PersistentParkingParams[tmpIndex])
...
function tfpp_writetofile()
	...
	if file:seek("end") > 0 then 
		print("Persistent parking: file save successful.") 
	else 
...
function DetermineTurnDirection(currentheading, desiredheading)
	...
	if not tfpp_bArrayInitialised and tfpp_bolOnTheGround == 1 and tfpp_InReplayMode == 0 and tfpp_datGSpd <= 4 and tfpp_intParkBrake == 1 then
		...
		if tfpp_bLoadPersistentParking then
			...
			if tfpp_PersistentParkingParams[tfpp_strCurrentAirport .. "XPOS"] == nil then
				...
				tfpp_fltFileHeading = tfpp_PersistentParkingParams[tfpp_strCurrentAirport .. "HPOS"]
				tfpp_fltFileHeading = tonumber(tfpp_fltFileHeading)
				...
				--adjust the plane heading
				
				--print("CH:" .. tfpp_fltCurrentHeading)
				--print("fltFH: " .. tfpp_fltFileHeading)
				
				fltHeadingAdjustment = math.abs(tfpp_fltFileHeading - tfpp_fltCurrentHeading)
				--print("fltHA: " .. fltHeadingAdjustment)

				local turndirection = DetermineTurnDirection(tfpp_fltCurrentHeading,tfpp_fltFileHeading)
				
				local turnrate = 135
				while fltHeadingAdjustment > 5 do
				
					
					if fltHeadingAdjustment <= turnrate then
						fltAdjAmt = fltHeadingAdjustment
					else
						fltAdjAmt = turnrate
					end
					...
					if turndirection == -1 then
						--turn left
						set("sim/flightmodel/position/R", (fltAdjAmt * -1))
					else
						--turn right
						set("sim/flightmodel/position/R", fltAdjAmt)
					end		

					--print("fltHA: " .. fltHeadingAdjustment .. " fltAdjAmt: " .. fltAdjAmt)
					--print("tfpp_fltCurrentHeading: " .. tfpp_fltCurrentHeading)
					fltHeadingAdjustment = fltHeadingAdjustment - fltAdjAmt
