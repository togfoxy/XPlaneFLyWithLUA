...
function tfpapi_DrawPAPILights(fltApproachAngle, degAngleChange)
	...
	local firstthreshold = 2.5
	local secondthreshold = 2.95
	--						3.2
	local thirdthreshold = 3.45
	local fourththreshold = 4.0
...
function tfpapi_main()
	...
	if tf_InReplayMode == 0 then	--make sure we're not in replay mode
		...
		if tf_PilotAltMSL >= 150 and tf_PilotAltMSL <= 451 then
			fltDistanceInNM = 1
		end
		if tf_PilotAltMSL >= 450 and tf_PilotAltMSL <= 751 then
			fltDistanceInNM = 2
		end
		if tf_PilotAltMSL >= 750 and tf_PilotAltMSL <= 1051 then
			fltDistanceInNM = 3
		end
		if tf_PilotAltMSL >= 1050 and tf_PilotAltMSL <= 1351 then
			fltDistanceInNM = 4
		end
		if tf_PilotAltMSL >= 1350 and tf_PilotAltMSL <= 1651 then
			fltDistanceInNM = 5
		end
		if tf_PilotAltMSL >= 1650 and tf_PilotAltMSL <= 1951 then
			fltDistanceInNM = 6
		end
		if tf_PilotAltMSL >= 1950 and tf_PilotAltMSL <= 2251 then
			fltDistanceInNM = 7
		end
		if tf_PilotAltMSL >= 2250 and tf_PilotAltMSL <= 2551 then
			fltDistanceInNM = 8
		end
		if tf_PilotAltMSL >= 2550 and tf_PilotAltMSL <= 2851 then
			fltDistanceInNM = 9
		end
		if tf_PilotAltMSL >= 2850 and tf_PilotAltMSL <= 4000 then
			fltDistanceInNM = 10
		end		
