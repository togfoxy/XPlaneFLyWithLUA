...
--v0.14
--		Experimented with GUI fade in/out
...
local fltFlightStartTimeSeconds 	 --tracks time at start of flight
...
dataref("tf_bolParkBrake","sim/flightmodel/controls/parkbrake")
...
function tfFSE_DrawStatusPanel()
	...
	if tf_fse_flying == 1 then
		fltFlightTimeSeconds = fse_flighttime
	...
	local fltFuelUsedOnFlightGALs = tf_common_functions.round((fltFuelUsedOnFlightKGs / 2.90),1)
...
function tfFSE_DrawDeltaState()
	...
	graphics.set_color(1, 1, 0, fltCurrentTransparency) --yellow
	graphics.draw_rectangle(x1,y1,x2,y2)
...
function tfFSE_DrawThings()
	...
	if MOUSE_X < x1 or MOUSE_X > x2 or MOUSE_Y < y1 or MOUSE_Y > y2 then
		--don't draw
		fltCurrentTransparency = fltCurrentTransparency - 0.02
...
function tfFSE_MouseClick()
	...
	if MOUSE_X >= x1 and MOUSE_X <= x2 and MOUSE_Y >= y1 and MOUSE_Y < y2 then
		...
		if MOUSE_X >= x1 and MOUSE_X <= x2 and MOUSE_Y >= y1 and MOUSE_Y < y2 then
				...
				if tf_fse_flying == 1 and bolCancelArmed == 0 and tf_bolOnTheGround == 1 and tf_datGSpd <= 5 and os.clock() >= (fltBeginLandedTimer + 4) and tf_bolParkBrake == 1 then
...
function tfFSE_Automation()	
	...
	if tf_bolOnTheGround == 1 and tf_datGSpd < 5 and tf_fse_flying == 1 and bolDeparted == 1 and tf_bolParkBrake == 1 then
...
--Raise a voice alarm if auto-resolved didn't work
	...
	if tf_bolOnTheGround == 1 and tf_fse_flying == 1 and tf_datGSpd < 5 and (os.clock() > (fltBeginLandedTimer + fltTimerForEndFlight + 30)) and bolFlightNotEndedAlertRaised == 0 and bolDeparted == 1 and tf_bolParkBrake == 1 then
